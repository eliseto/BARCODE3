...
class account_payment(models.TransientModel):
    ...
    def _create_payments(self):
        res = super(account_payment, self)._create_payments()
        ...
        # if invoices:
        #     invoices.manual_currency_rate_active = True
        #     invoices.manual_currency_rate = self.manual_currency_rate
    ...
    def default_get(self, default_fields):
        ...
        if len(invoices or []) > 1:
            ...
            if any(inv.manual_currency_rate_active == False for inv in invoices):
                ...
                if any((1.0/inv.manual_currency_rate) != rate for inv in invoices):
                    raise ValidationError(_("Selected invoice to make payment have not similer currency or currency rate is not same.\n Make sure selected invoices have same currency and same manual currency rate."));
        rec.update({
        ...
        for wizard in self:
            if wizard.source_currency_id == wizard.currency_id:
                ...
                if self.manual_currency_rate_active and self.manual_currency_rate > 0:
                    ...
                    currency_rate = 1.0/self.manual_currency_rate
...
class AccountPayment(models.Model):
            ...
            for res in query_res:
                ...
                if move_currency == currency and move_currency != company.currency_id:
                    ...
                    if not inv.manual_currency_rate_active:
                        ...
                        total += res['residual_currency'] * (1.0/inv.manual_currency_rate)
        return total


    @api.depends('invoice_ids', 'amount', 'payment_date', 'currency_id', 'payment_type', 'manual_currency_rate', 'manual_currency_rate_active')
    def _compute_payment_difference(self):
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        if self.manual_currency_rate_active and self.manual_currency_rate > 0:
            ...
            currency_rate = self.company_id.currency_id.rate / compute_rate
    ...
    def _synchronize_to_moves(self, changed_fields):
        ...
        if not any(field_name in changed_fields for field_name in (
            ...
            'currency_id', 'partner_id', 'destination_account_id', 'partner_bank_id', 'manual_currency_rate', 'manual_currency_rate_active'
        )):
            return
