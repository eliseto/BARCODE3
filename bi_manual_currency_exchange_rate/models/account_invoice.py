...
class stock_move(models.Model):
			...
			for line in self:
				if line.purchase_line_id :
					if line.purchase_line_id.order_id.purchase_manual_currency_rate_active:
						compute_rate = 1.0 / line.purchase_line_id.order_id.purchase_manual_currency_rate
						price_unit = line.purchase_line_id.order_id.currency_id.round((line.purchase_line_id.price_unit)/compute_rate)
	...
	def _prepare_account_move_line(self, qty, cost, credit_account_id, debit_account_id, description):
		...
		if self.purchase_line_id.order_id.purchase_manual_currency_rate_active:
			compute_rate = 1.0 / self.purchase_line_id.order_id.purchase_manual_currency_rate
			debit_value = self.purchase_line_id.order_id.currency_id.round((self.purchase_line_id.price_unit*qty)/ compute_rate or 1)
			credit_value = debit_value
		...
		if self.sale_line_id.order_id.sale_manual_currency_rate_active:
			compute_rate = 1.0 / self.sale_line_id.order_id.sale_manual_currency_rate
			credit_value = 	self.sale_line_id.order_id.currency_id.round((self.sale_line_id.price_unit*qty)/compute_rate or 1)
...
class account_invoice_line(models.Model):
	...
	def _get_fields_onchange_subtotal_model(self, price_subtotal, move_type, currency, company, date):
		...
		if self.move_id.manual_currency_rate_active and self.move_id.manual_currency_rate > 0:
			compute_rate = 1.0 / self.move_id.manual_currency_rate
			currency_rate = company.currency_id.rate / compute_rate
	...
	def _onchange_amount_currency(self):
		for line in self:
			...
			if line.move_id.manual_currency_rate_active and line.move_id.manual_currency_rate > 0:
				compute_rate = 1.0 / line.move_id.manual_currency_rate
				currency_rate = company.currency_id.rate /compute_rate
	...
	def _get_computed_price_unit(self):
		...
		if self.move_id.manual_currency_rate_active and self.move_id.manual_currency_rate > 0:
			price_unit = res
			compute_rate = 1.0 / self.move_id.manual_currency_rate
			currency_rate = compute_rate/self.move_id.company_id.currency_id.rate
...
class account_invoice(models.Model):
	...
	manual_currency_rate_active = fields.Boolean('Tasa de cambio manual')
	...
	def _recompute_tax_lines(self, recompute_tax_base_amount=False):
		...
		for taxes_map_entry in taxes_map.values():
			...
			if self.manual_currency_rate_active and self.manual_currency_rate > 0:
				compute_rate = 1.0 / self.manual_currency_rate
				currency_rate = self.company_currency_id.rate / compute_rate
